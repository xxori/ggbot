...
class Moderation(discord.Cog):
    ...
    @utils.is_leader()
    @commands.command(name="send", aliases=["s"], description="DM a server member", hidden=True)
    ...
    @utils.is_leader()
    @commands.command(brief='Execute command as another user.', usage='[user] <command>', hidden=True)
